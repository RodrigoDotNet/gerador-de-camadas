...
namespace DataDrain.UI.WinForm.SuportForms
{
    ...
    {
        private BindingList<StoredProcedureParameter> _parametros;
        ...
        public DatabaseObjectInfo ObjetoSelecionado { get; private set; }
        ...
        private void frmPesquisaManual_Load(object sender, EventArgs e)
        {
            ...
            _parametros = new BindingList<StoredProcedureParameter>();
        ...
        private void txtConsulta_TextChanged(object sender, EventArgs e)
        {
            ...
            if (txtConsulta.Text.Contains("@"))
            {
                ...
                {
                    var nomeParametro = consulta.Substring(posicao,
                        RetornaPosicaoFinalNomeParametro((posicao + 1), consulta));

                    if (_parametros.Count(p => p.ParameterName == nomeParametro) == 0)
                    {
                        _parametros.Add(new StoredProcedureParameter
                        {
        ...
        {
            var palavrasRestritas = new List<string>
            {
                "insert",
                "update",
                "delete",
        ...
        private void PreparaGridViewParametros()
        {
            ...
            var lista = new DataGridViewComboBoxColumn
            {
                HeaderText = "Tipo de Parâmetro",
                Name = "cbTipoParan",
                Width = 100,
                Visible = true,
                DataSource = _tiposDadosParametros,
                DisplayMember = "Key",
                ValueMember = "Value",
                ValueType = typeof (KeyValuePair<string, string>),
                DataPropertyName = "ParameterDotNetType"
            };
            var valor = new DataGridViewMaskedTextColumn
            {
                HeaderText = "Valor",
                Name = "Valor",
                Width = 120,
        ...
        private void CarregaGridViewParametros()
        {
            ...
            foreach (var param in _parametros)
            {
                ...
                var defineNull = new DataGridViewCheckBoxCell {Value = param.DefaultNull};
                var lista = new DataGridViewComboBoxCell
                {
                    DataSource = _tiposDadosParametros,
                    DisplayMember = "Key",
                    ValueMember = "Value",
