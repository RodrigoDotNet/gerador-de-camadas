...
using DataDrain.Library.ORM.Caching;

namespace DataDrain.Library.ORM
{
    public static class Map
    {
        ...
        private static Dictionary<string, Action<T, object>> RetornaSetters<T>(IEnumerable<PropertyInfo> camposValidos)
        {
            ...
            {
                var mapaCache = map[typeof(T).FullName];

                if (mapObjAtual.Equals(mapaCache.Count))
                {
                    return mapaCache;
                }
                else
                {
                    map.Remove(typeof(T).FullName);
                }
            }
